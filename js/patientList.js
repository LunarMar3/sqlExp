...
function displayPatients(patients) {
    ...
    if (patients.length === 0) {
        patientListTbody.innerHTML = '<tr><td colspan="7">No patients found.</td></tr>';
        return;
    }
    ...
    patients.forEach(patient => {
        ...
        patientRow.innerHTML = `
            ...
            <td><button onclick="openEditModal('${patient.patientID}', '${patient.medicalHistory || ''}', '${patient.roomID || ''}')">Edit</button></td>
        `;
...
// 打开编辑模态窗口
function openEditModal(patientID, medicalHistory, roomID) {
...
function savePatientUpdate() {
    ...
    const updatedMedicalHistory = document.getElementById('edit-medicalHistory').value;
    const updatedRoomID = document.getElementById('edit-roomID').value;
